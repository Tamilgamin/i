...
interface AppSettings {
  ...
  lowTempThreshold: number; // Relay Low
  highTempThreshold: number; // Relay High
  lowHumidityThreshold: number; // Relay Low
  highHumidityThreshold: number; // Relay High
  alarmLowTemp: number;
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (isSimulating) {
      interval = setInterval(() => {
        ...
        const newHum = 50 + (Math.random() - 0.5) * 10;
        const now = new Date();
  ...
  const debouncedPublish = useCallback((topic: string, message: string, delay: number = 200) => {
  ...
  useEffect(() => {
    if (temp !== null || humidity !== null) {
      const isTempOut = temp !== null && (temp < settings.alarmLowTemp || temp > settings.alarmHighTemp);
      const isHumidityOut = humidity !== null && (humidity < settings.alarmLowHum || humidity > settings.alarmHighHum);
  ...
  }, [temp, humidity, triggerAlarm, settings.alarmLowTemp, settings.alarmHighTemp, settings.alarmLowHum, settings.alarmHighHum, isAlarmActive, stopAlarm]);
  ...
  const updateAlarmTempThresholds = (low: number, high: number) => {
    const safeLow = Math.min(low, high - 0.1);
    const safeHigh = Math.max(high, low + 0.1);
    setSettings(s => ({ ...s, alarmLowTemp: safeLow, alarmHighTemp: safeHigh }));
  };

  const updateAlarmHumidityThresholds = (low: number, high: number) => {
    const safeLow = Math.min(low, high - 1);
    const safeHigh = Math.max(high, low + 1);
    setSettings(s => ({ ...s, alarmLowHum: safeLow, alarmHighHum: safeHigh }));
  };
          ...
          <AnimatePresence mode="wait">
            {activeTab === 'home' ? (
              ...
              >
                ...
                <div className="space-y-4">
                  <div className="flex items-center space-x-2 px-1">
                    <div className="w-1.5 h-1.5 rounded-full bg-zinc-700" />
                    <h3 className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest">Relay Control Limits (Incubator)</h3>
                  </div>
                  ...
                  <div className="bg-zinc-900/50 border border-zinc-800 p-5 rounded-3xl backdrop-blur-md space-y-5">
                    <div className="flex justify-between items-center">
                      <div className="flex items-center space-x-3">
                        ...
                        <span className="text-xs font-bold text-zinc-200 uppercase tracking-tight block">Heater Range</span>
                      </div>
              ...
              >
                ...
                <div className="h-64 w-full bg-zinc-900/30 rounded-3xl p-4 border border-zinc-800">
                  <ResponsiveContainer width="100%" height="100%">
              ...
              >
                <div className="space-y-3">
                  <div className="flex items-center space-x-2 px-1">
                    <div className="w-1.5 h-1.5 rounded-full bg-zinc-700" />
                    <h3 className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest">Alarm & Notification Settings</h3>
                  </div>
                  {/* Toggles Grid */}
                  <div className="grid grid-cols-2 gap-3">
                    {/* Alarm Toggle */}
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div className="space-y-1">
                          ...
                          <input 
                            type="number" 
                            step="0.1"
                            value={settings.alarmLowTemp}
                            onChange={(e) => updateAlarmTempThresholds(parseFloat(e.target.value) || 0, settings.alarmHighTemp)}
                        ...
                        <div className="space-y-1">
                          ...
                          <input 
                            type="number" 
                            step="0.1"
                            value={settings.alarmHighTemp}
                            onChange={(e) => updateAlarmTempThresholds(settings.alarmLowTemp, parseFloat(e.target.value) || 0)}
                    ...
                    <div className="bg-zinc-900/50 p-4 rounded-2xl border border-zinc-800 space-y-3">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div className="space-y-1">
                          ...
                          <input 
                            type="number" 
                            value={settings.alarmLowHum}
                            onChange={(e) => updateAlarmHumidityThresholds(parseFloat(e.target.value) || 0, settings.alarmHighHum)}
                        ...
                        <div className="space-y-1">
                          ...
                          <input 
                            type="number" 
                            value={settings.alarmHighHum}
                            onChange={(e) => updateAlarmHumidityThresholds(settings.alarmLowHum, parseFloat(e.target.value) || 0)}
                  ...
                  <div className="bg-zinc-900/50 p-5 rounded-[2rem] border border-zinc-800 flex items-center justify-between">
                    <div>
                      <p className="text-zinc-400 text-sm font-medium">Simulation Mode</p>
                      <p className="text-zinc-500 text-xs">Test UI without MQTT broker</p>
                    </div>
                    <button 
                      onClick={() => setIsSimulating(!isSimulating)}
                      className={cn(
                        "w-14 h-8 rounded-full transition-colors relative",
                        isSimulating ? "bg-emerald-500" : "bg-zinc-800"
                      )}
                    >
                      <div className={cn(
